...
const CardList = ({ isDeckbuilding, preFilter, preFilterList }: CardListProps) => {
    ...
    if (typeof tcg === "string" && typeof setType === "string") {
      fetchCardsByTcg(tcg, setType)
        .then((data) => {
          setFilters({});
          if (preFilter) {
            console.log("Applying pre-filters:", preFilterList);
            const filtered = data.filter((card:CardOnePiece) =>
              preFilterList?.color.includes(String(card.color).toLowerCase())
              && !preFilterList?.category.includes(String(card.category).toLowerCase()
            ));
